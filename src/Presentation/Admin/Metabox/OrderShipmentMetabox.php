...
final class OrderShipmentMetabox
{
    ...
    public function enqueueAssets(string $hook): void
    {
        ...
        wp_enqueue_style('dexpress-design-system', DEXPRESS_PLUGIN_URL . 'assets/css/admin-design-system.css', [], DEXPRESS_VERSION);
        wp_enqueue_style('dexpress-admin-metabox', DEXPRESS_PLUGIN_URL . 'assets/css/admin-metabox.css', ['dashicons', 'dexpress-design-system'], DEXPRESS_VERSION);
    ...
    public function render(\WP_Post|\WC_Order $postOrOrder): void
    {
        ...
        echo '<div id="dexpress-shipment-root" class="dexpress-metabox">';
    ...
    private function renderWizard(\WC_Order $order, array $senderLocations, ?Shipment $prefillShipment = null): void
    {
        ...
        echo '<div class="dexpress-state dexpress-state--wizard" data-state="wizard"><div class="dexpress-steps">'
            . '<span class="dexpress-step is-active" data-step="1">' . esc_html__('1. Paketi', 'dexpress-woocommerce') . '</span>'
            . '<span class="dexpress-step" data-step="2">' . esc_html__('2. Opcije pošiljke', 'dexpress-woocommerce') . '</span>'
            . '<span class="dexpress-step" data-step="3">' . esc_html__('3. Pregled i štampa', 'dexpress-woocommerce') . '</span></div>';
        echo '<section class="dexpress-step-panel" data-step="1"><h4>' . esc_html($wizardTitle) . '</h4><div id="dexpress-package-cards" class="dexpress-package-cards"></div><div class="dexpress-package-actions"><button type="button" class="button" id="dexpress-remove-package">− ' . esc_html__('Ukloni paket', 'dexpress-woocommerce') . '</button><button type="button" class="button button-secondary" id="dexpress-add-package">+ ' . esc_html__('Dodaj paket', 'dexpress-woocommerce') . '</button></div></section>';
        echo '<section class="dexpress-step-panel" data-step="2" hidden><h4>' . esc_html__('Opcije pošiljke', 'dexpress-woocommerce') . '</h4>';
        $destination = $this->destinationLine($order);
        if (DexpressPackageShopShippingMethod::orderUsesDexpressPackageShop($order) && $destination !== '') {
            echo '<p class="dexpress-package-shop-destination">' . esc_html__('Dostava na:', 'dexpress-woocommerce') . ' <strong>' . esc_html($destination) . '</strong></p>';
        }
        echo '<p><label class="dexpress-field-label" for="dexpress-sender-location">' . esc_html__('Lokacija pošiljaoca', 'dexpress-woocommerce') . '</label><select id="dexpress-sender-location" class="widefat">';
        ...
        echo '</select></p>';
        echo '<p><label class="dexpress-field-label" for="dexpress-delivery-type">' . esc_html__('Tip dostave', 'dexpress-woocommerce') . '</label><select id="dexpress-delivery-type" class="widefat">';
        ...
        echo '</select></p>';
        echo '<p><label class="dexpress-field-label" for="dexpress-payment-type">' . esc_html__('Način plaćanja', 'dexpress-woocommerce') . '</label><select id="dexpress-payment-type" class="widefat">';
        foreach (PaymentType::cases() as $type) {
            $selected = (string) $type->value === $defaults['payment_type'] ? ' selected' : '';
            echo '<option value="' . esc_attr((string) $type->value) . '"' . $selected . '>' . esc_html($type->label()) . '</option>';
        }
        echo '</select></p>';
        echo '<p><label class="dexpress-field-label" for="dexpress-return-doc">' . esc_html__('Povraćaj dokumenta', 'dexpress-woocommerce') . '</label><select id="dexpress-return-doc" class="widefat">';
        ...
        echo '</select></p><p><label><input type="checkbox" id="dexpress-self-drop-off" value="1"' . $checked . '> ' . esc_html__('Lično predajem kuriru (self drop-off)', 'dexpress-woocommerce') . '</label></p>';
        echo '<p><label class="dexpress-field-label" for="dexpress-content">' . esc_html__('Sadržaj pošiljke', 'dexpress-woocommerce') . ' <span class="dexpress-req">*</span></label><input type="text" id="dexpress-content" class="widefat" maxlength="50"></p>';
        echo '<p><label class="dexpress-field-label" for="dexpress-note">' . esc_html__('Napomena', 'dexpress-woocommerce') . '</label><input type="text" id="dexpress-note" class="widefat" maxlength="150"></p></section>';
        echo '<section class="dexpress-step-panel" data-step="3" hidden><h4>' . esc_html__('Pregled i štampa', 'dexpress-woocommerce') . '</h4><div id="dexpress-step3-summary" class="dexpress-step3-summary"></div></section>';
        echo '<div id="dexpress-wizard-error" class="dexpress-wizard-error" role="alert" hidden></div><div id="dexpress-wizard-result" class="dexpress-wizard-result" aria-live="polite"></div><div class="dexpress-wizard-nav"><button type="button" class="button" id="dexpress-step-back" hidden>' . esc_html__('Nazad', 'dexpress-woocommerce') . '</button><button type="button" class="button button-primary" id="dexpress-step-next">' . esc_html__('Dalje', 'dexpress-woocommerce') . '</button><button type="button" class="button button-primary" id="dexpress-print-label" hidden>' . esc_html__('Štampaj nalepnicu', 'dexpress-woocommerce') . '</button></div></div>';
    }

    private function renderPendingSend(Shipment $shipment): void
    {
        echo '<div class="dexpress-state dexpress-state--draft" data-state="pending_send" data-shipment-id="' . esc_attr((string) $shipment->id()) . '" data-label-url="' . esc_url($this->labelUrlForShipment($shipment)) . '">';
        echo '<h4>' . esc_html__('Pošiljka čeka slanje', 'dexpress-woocommerce') . '</h4>';
        echo '<p class="dexpress-draft-warning">' . esc_html__('Nalepnica je odštampana. Spakujte pošiljku, zalepite nalepnicu i kliknite Pošalji.', 'dexpress-woocommerce') . '</p>';
        echo '<p><strong>' . esc_html__('Kod za praćenje:', 'dexpress-woocommerce') . '</strong> <code>' . esc_html($shipment->trackingCode()) . '</code></p>';
        echo '<div id="dexpress-wizard-result" class="dexpress-wizard-result" aria-live="polite"></div><div class="dexpress-wizard-nav"><button type="button" class="button" id="dexpress-edit-shipment">' . esc_html__('Izmeni podatke', 'dexpress-woocommerce') . '</button><button type="button" class="button" id="dexpress-reprint-label">' . esc_html__('Štampaj ponovo', 'dexpress-woocommerce') . '</button><button type="button" class="button button-primary" id="dexpress-send-shipment">' . esc_html__('Pošalji u D-Express', 'dexpress-woocommerce') . '</button></div></div>';
    }
    ...
    private function renderCreated(Shipment $shipment, \WC_Order $order): void
    {
        ...
        echo '<div class="dexpress-state dexpress-state--created" data-state="created">';
        echo '<div class="dexpress-sent-banner"><span class="dexpress-env-badge ' . ($isTest ? 'is-test' : 'is-production') . '">' . esc_html($isTest ? 'TEST' : 'PRODUCTION') . '</span> ' . esc_html__('Pošiljka uspešno poslata u D-Express', 'dexpress-woocommerce') . '</div>';

        echo '<div class="dexpress-sent-body">';
        echo '<div class="dexpress-sent-parties">';
        echo '<div class="dexpress-sent-party"><span class="dexpress-sent-label">' . esc_html__('Kod', 'dexpress-woocommerce') . ':</span> <code>' . esc_html($trackingCode) . '</code></div>';
        echo '<div class="dexpress-sent-party"><span class="dexpress-sent-label">' . esc_html__('Od', 'dexpress-woocommerce') . ':</span> <span>' . esc_html($senderText) . '</span></div>';
        echo '<div class="dexpress-sent-party"><span class="dexpress-sent-label">' . esc_html__('Za', 'dexpress-woocommerce') . ':</span> <span>' . esc_html($recipientText) . '</span>';
        if ($isPackageShopOrder) {
            echo ' <span class="dexpress-pill">' . esc_html($packageShopLocationType) . '</span>';
            if ($packageShopLocationName !== '') {
                echo ' <span>' . esc_html($packageShopLocationName) . '</span>';
            }
        }
        echo '</div>';
        echo '<div class="dexpress-sent-party"><span class="dexpress-sent-label">' . esc_html__('Detalji', 'dexpress-woocommerce') . ':</span> <span>' . esc_html((string) $packageCount . ' / ' . number_format($totalMassGrams / 1000, 2, ',', '') . ' kg · ' . $shipment->deliveryType->label() . ' · ' . $shipment->paymentType->label()) . '</span></div>';
        echo '</div>';

        $allocations = $this->packageItemAllocations((int) $shipment->id());
        echo '<div class="dexpress-sent-packages"><ul>';
        foreach ($shipment->packages as $pkg) {
        ...
        echo '<div class="dexpress-sent-timeline">';
        echo '<h4>' . esc_html__('Status pošiljke', 'dexpress-woocommerce') . '</h4>';
        echo '<ol class="dexpress-status-timeline">';
        foreach ($timeline as $step) {
            ...
            $date = $step['occurred_at'] !== ''
                ...
                : '';
            echo '<li class="dexpress-status-step ' . esc_attr($stateClass) . '"><span class="dexpress-status-dot" aria-hidden="true"></span><div class="dexpress-status-content"><strong>' . esc_html($step['label']) . '</strong>';
            if ($date !== '') {
        ...
        echo '<div class="dexpress-created-actions">';
        ...
        echo '<button type="button" class="button dexpress-copy-track" data-track="' . esc_attr($trackingCode) . '">' . esc_html__('Kopiraj kod za praćenje', 'dexpress-woocommerce') . '</button>';
        ...
        echo '</div>';
        echo '<div id="dexpress-wizard-result" class="dexpress-wizard-result" aria-live="polite"></div>';
        echo '</div>';
    }
