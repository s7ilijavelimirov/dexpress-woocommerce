...
final class SyncTownsService
{
    private const SYNC_KEY       = 'sync.last_towns';
    private const FIRST_SYNC_DATE = '20000101000000';

    public function __construct(
        private readonly DExpressApiClient $apiClient,
        private readonly TownRepository    $repository,
    ...
    public function sync(): SyncResult
    {
        try {
            ...
            $rows  = $this->apiClient->get('/data/towns', ['date' => $since]);
            $stats = $this->repository->upsertBatch($rows);

            $this->options->set(self::SYNC_KEY, current_time('YmdHis'));
            $this->options->save();

            return SyncResult::success('towns', $stats);
        } catch (\Throwable $e) {
            return SyncResult::failure('towns', $e->getMessage());
        }
    }
}
